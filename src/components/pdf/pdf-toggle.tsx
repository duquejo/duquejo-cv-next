  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant={variant} size="icon" className={cn('h-10 w-10', className)}>
          <ImageDown className="h-8 w-8 lg:h-10 lg:w-10" aria-label={buttonTitle} />
          <span className="sr-only">{buttonTitle}</span>
        </Button>
      </PopoverTrigger>
      <PopoverContent role="dialog" aria-label={formTitle}>
        ...
        <PdfForm button={formButtonText} buttonLoading={formButtonTextLoading} />
      </PopoverContent>
    </Popover>
  );
};
